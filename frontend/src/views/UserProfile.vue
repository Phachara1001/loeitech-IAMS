...
function applyUserData(data) {
  currentUser.value = {
    ...
    position: data.position || 'ครูประจำแผนก',
    email: data.email,
    phone: data.phone || '-',
    role: data.role,
...
<template>
  <div>
    ...
    <div class="min-h-screen bg-[#F4F7F5] font-sarabun p-4 sm:p-6 lg:p-8 print:hidden">
        ...
        <template v-else>
          ...
          <div class="relative overflow-hidden rounded-2xl bg-[#072415] text-white shadow-xl">
            ...
            <div class="absolute inset-0 pointer-events-none overflow-hidden">
              <div
                class="absolute -top-20 -right-20 w-[500px] h-[500px] rounded-full bg-[#1B5E3C] opacity-75 blur-[90px]">
              </div>
              <div
                class="absolute top-1/2 -left-20 w-[400px] h-[400px] rounded-full bg-[#288252] opacity-40 blur-[80px]">
              </div>
              <div
            ...
            <div class="relative z-10 p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
              <div class="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
                ...
                <div class="relative group cursor-pointer"
                  :class="{ 'pointer-events-none opacity-70': isUploadingAvatar }" @click="triggerFileInput"
                  title="คลิกเพื่อเปลี่ยนรูปโปรไฟล์">
                  <!-- Glow Effect -->
                  <div
                    class="absolute -inset-2 bg-gradient-to-r from-[#1B5E3C] via-[#288252] to-[#0F3D26] rounded-full blur-md opacity-80 animate-spin-slow">
                  </div>
                  ...
                  <!-- กรณีมีรูปภาพแล้ว -->
                  <img v-if="currentUser.avatar" :src="currentUser.avatar" alt="โปรไฟล์ผู้ใช้"
                  ...
                  <div v-if="isUploadingAvatar"
                    class="absolute inset-0 rounded-full bg-black/70 flex items-center justify-center z-20">
                    <Loader2 class="w-6 h-6 text-emerald-300 animate-spin" />
                  </div>

                  <!-- Overlay ตอน Hover ว่า "เปลี่ยนรูป" -->
                  <div v-else
                    class="absolute inset-0 rounded-full bg-black/60 opacity-0 group-hover:opacity-100 transition flex flex-col items-center justify-center text-white text-xs font-semibold backdrop-blur-[2px] z-20">
                ...
                <div>
                  <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-2">
                    ...
                    <span
                      class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-400/20 text-emerald-200 border border-emerald-400/30 backdrop-blur-md">
                      <ShieldCheck class="w-3.5 h-3.5 text-emerald-300" />
                      สิทธิ์: {{ currentUser.role }}
                    </span>
                  </div>
                  <p
                    class="text-emerald-100/90 text-base font-medium flex items-center justify-center sm:justify-start gap-2">
            ...
            <div
              class="relative z-10 bg-black/20 border-t border-white/10 px-6 sm:px-8 py-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm text-emerald-100/80">
              ...
              <div class="flex items-center gap-2">
                ...
                <button @click="openChangePasswordModal" title="เปลี่ยนรหัสผ่าน"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="bg-white rounded-xl p-5 border border-slate-200/80 shadow-sm flex items-center justify-between">
              <div>
                ...
                <p class="text-3xl font-bold text-[#072415] mt-1">{{ totalAssets }} <span
                    class="text-sm font-normal text-slate-500">รายการ</span></p>
              </div>
            ...
            <div class="bg-white rounded-xl p-5 border border-slate-200/80 shadow-sm flex items-center justify-between">
              <div>
                ...
                <p class="text-3xl font-bold text-emerald-600 mt-1">{{ normalAssets }} <span
                    class="text-sm font-normal text-slate-500">รายการ</span></p>
              </div>
            ...
            <div class="bg-white rounded-xl p-5 border border-slate-200/80 shadow-sm flex items-center justify-between">
              <div>
                ...
                <p class="text-3xl font-bold text-amber-600 mt-1">{{ repairAssets }} <span
                    class="text-sm font-normal text-slate-500">รายการ</span></p>
              </div>
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                  <tr v-for="asset in myAssets" :key="asset.id" class="hover:bg-slate-50/80 transition">
                    <td class="py-4 px-4">
                      ...
                      <p class="text-xs text-slate-500 font-mono mt-0.5">{{ asset.id }} · SN: {{ asset.serialNumber }}
                      </p>
                    </td>
                    ...
                    <td class="py-4 px-4 text-center whitespace-nowrap">
                      ...
                      <span v-else-if="asset.status === 'repair'"
                    ...
                    <td class="py-4 px-4 text-right whitespace-nowrap">
                      <button @click="openDetailModal(asset)"
      ...
      <div v-if="isChangePasswordModalOpen"
        ...
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl relative">
          ...
          <div v-if="passwordError"
            class="mb-4 p-3 rounded-xl bg-red-50 border border-red-200 text-red-600 text-sm flex items-center gap-2">
            <XCircle class="w-4 h-4 shrink-0" />
            <span>{{ passwordError }}</span>
          </div>

          <div v-if="passwordSuccess"
            class="mb-4 p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm flex items-center gap-2">
        ...
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl relative">
          ...
          <form @submit.prevent="handleSaveProfile" class="space-y-4">
            ...
            <div>
              ...
              <input v-model="editProfileForm.position" type="text"
                class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#1B5E3C]/20 focus:border-[#1B5E3C]" />
            </div>
    ...
    <div class="hidden print:block font-sarabun bg-white text-black p-10 max-w-[210mm] mx-auto min-h-screen">
      <div class="text-center mb-8">
        <h1 class="text-3xl font-bold mb-3">ใบรับรองการครอบครองครุภัณฑ์</h1>
        <h2 class="text-xl font-bold mb-8">วิทยาลัยเทคนิคเลย</h2>
      </div>

      <div class="mb-6 leading-relaxed text-base">
        <p class="mb-2">
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;หนังสือรับรองฉบับนี้ให้ไว้เพื่อแสดงว่า <strong>{{
            currentUser.name
          }}</strong> ตำแหน่ง <strong>{{ currentUser.position }}</strong>
          เป็นผู้ครอบครองและรับผิดชอบดูแลครุภัณฑ์ของวิทยาลัยเทคนิคเลย จำนวนทั้งสิ้น <strong>{{ myAssets.length
          }}</strong>
          รายการ ดังรายละเอียดต่อไปนี้:
        </p>
      </div>
      ...
      <table class="w-full border-collapse border border-black text-sm mb-8">
        <thead>
          <tr class="bg-gray-100">
            <th class="border border-black py-2.5 px-3 text-center w-12 font-bold">ลำดับ</th>
            <th class="border border-black py-2.5 px-3 text-center w-32 font-bold">รหัสครุภัณฑ์</th>
            <th class="border border-black py-2.5 px-3 text-left font-bold">รายการครุภัณฑ์</th>
            <th class="border border-black py-2.5 px-3 text-center w-32 font-bold">วันที่รับมอบ</th>
            <th class="border border-black py-2.5 px-3 text-center w-24 font-bold">สถานะ</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(asset, index) in myAssets" :key="asset.id">
            <td class="border border-black py-2 px-3 text-center">{{ index + 1 }}</td>
            <td class="border border-black py-2 px-3 text-center font-mono text-xs">{{ asset.id }}<br /><span
                class="text-[10px] text-gray-500">SN: {{ asset.serialNumber || '-' }}</span></td>
            <td class="border border-black py-2 px-3 font-medium">{{ asset.name }}<br /><span
                class="text-xs text-gray-600 font-normal">หมวดหมู่: {{ asset.category }}</span></td>
            <td class="border border-black py-2 px-3 text-center">{{ asset.receivedDate || '-' }}</td>
            <td class="border border-black py-2 px-3 text-center">
              {{ asset.status === 'normal' ? 'ปกติ' : (asset.status === 'repair' ? 'แจ้งซ่อม/ชำรุด' : asset.status) }}
            </td>
          </tr>
          <tr v-if="myAssets.length === 0">
            <td colspan="5" class="border border-black py-8 text-center text-gray-500 italic">
              ไม่มีรายการครุภัณฑ์ที่ถือครอง
            </td>
          </tr>
        </tbody>
      </table>

      <div class="flex justify-end mt-16 text-base">
        <div class="text-center w-72">
          <p class="mb-4">ออกให้ ณ วันที่ {{ new Date().toLocaleDateString('th-TH', {
            year: 'numeric', month: 'long',
            day:
              'numeric'
          }) }}</p>
          <p class="mt-12 pt-2 border-t border-gray-400 border-dashed inline-block px-4">
            ผู้พิมพ์เอกสาร: {{ currentUser.name }}
          </p>
        </div>
      </div>
    </div>
