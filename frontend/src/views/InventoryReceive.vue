...
<template>
  <div class="min-h-screen bg-slate-100/80 text-slate-800 p-4 sm:p-6 md:p-10 font-sans">
    
    <!-- Guard Notice -->
    <div v-if="!isAuthorized" class="max-w-2xl mx-auto mt-12 bg-white rounded-3xl p-8 shadow-xl text-center border-2 border-rose-200">
    ...
    <div v-else class="max-w-7xl mx-auto space-y-8">
      ...
      <div class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-200/80 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div class="flex items-center gap-5">
          <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white flex items-center justify-center shadow-lg shadow-emerald-500/20 shrink-0">
            <PackagePlus class="w-9 h-9" />
          </div>
          <div>
            <div class="flex items-center gap-2">
              <span class="bg-emerald-100 text-emerald-800 text-sm font-extrabold px-3 py-1 rounded-full border border-emerald-200">
                แบบ พ.3102-8 หน้า 1
              </span>
              <span class="text-slate-400 text-sm font-medium">| ระบบสต๊อกหลัก</span>
            </div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 mt-1">บันทึกการรับพัสดุเข้าคลัง</h1>
          </div>
        </div>

        <div class="flex items-center gap-3 bg-slate-50 p-2.5 rounded-2xl border border-slate-200 self-start md:self-auto">
          <ShieldCheck class="w-5 h-5 text-emerald-600 ml-2" />
          <span class="text-base font-bold text-slate-700">{{ currentUser.name }}</span>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <form @submit.prevent="handleSaveStockReceive" class="lg:col-span-8 bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200/80 space-y-8">
          ...
          <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
              ...
              <div>
                ...
                <select 
                  v-model="formData.acquisitionMethod" 
                  class="w-full bg-slate-50 border-2 border-slate-200 rounded-2xl p-4 text-slate-900 font-bold text-lg focus:outline-none focus:border-emerald-600 focus:bg-white transition-all cursor-pointer"
                >
          ...
          <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
              ...
              >
                <input type="radio" v-model="itemSourceType" value="EXISTING" class="w-6 h-6 text-emerald-600 accent-emerald-600" />
                <div>
                  <span class="block text-lg font-extrabold text-slate-900">📦 วัสดุที่มีในคลังอยู่แล้ว</span>
                  <span class="text-sm font-medium text-slate-500">เลือกพัสดุเดิมเพื่อเพิ่มจำนวนสต๊อก</span>
                </div>
              </label>
              ...
              >
                <input type="radio" v-model="itemSourceType" value="NEW" class="w-6 h-6 text-emerald-600 accent-emerald-600" />
                <div>
                  <span class="block text-lg font-extrabold text-slate-900">✨ พัสดุ/วัสดุใหม่ (ยังไม่มีในระบบ)</span>
                  <span class="text-sm font-medium text-slate-500">กรอกชื่อเพื่อลงทะเบียนเข้าคลังใหม่</span>
                </div>
              </label>
            </div>
            ...
            <div v-if="itemSourceType === 'EXISTING'" class="mt-6">
              ...
              <select 
                v-model="formData.selectedItemId" 
                required
            ...
            <div v-else class="mt-6 p-6 bg-slate-50 rounded-2xl border-2 border-emerald-200 space-y-4">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select v-model="formData.newItemCategory" class="w-full bg-white border-2 border-slate-300 rounded-2xl p-3.5 text-slate-900 font-bold text-base focus:border-emerald-600">
                ...
                <div>
                  ...
                  <input 
                    v-model="formData.newItemUnit" 
                    type="text" 
                    required
                    placeholder="เช่น อัน, กล่อง, แฟ้ม, ด้าม" 
            ...
            <div class="mt-6">
              ...
              <textarea 
                ...
                rows="2"
                placeholder="เช่น ใบสั่งซื้อเลขที่ PO-2569/088..."
                class="w-full bg-slate-50 border-2 border-slate-200 rounded-2xl p-4 text-slate-900 font-medium text-base focus:outline-none focus:border-emerald-600 focus:bg-white transition-all"
              ></textarea>
            </div>
          </div>
        ...
        <div class="lg:col-span-4 space-y-6">
          <div class="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-3xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
            ...
            <div class="mt-6 space-y-6">
              <div>
                ...
                <span v-if="itemSourceType === 'NEW'" class="inline-block mt-2 px-2.5 py-0.5 bg-emerald-500/20 text-emerald-300 rounded-md text-xs font-bold border border-emerald-500/30">
                  ✨ พัสดุรายการใหม่
                </span>
              </div>
              ...
              <div class="bg-slate-800/80 p-5 rounded-2xl border border-slate-700/80 space-y-3">
                <div class="flex items-center justify-between text-base">
                  ...
                  <span class="font-mono font-bold text-white text-lg">
                    {{ itemSourceType === 'EXISTING' && selectedExistingItem ? selectedExistingItem.current_stock : 0 }} {{ displayUnit }}
                  </span>
                </div>
                
                <div class="flex items-center justify-between text-base text-emerald-400 font-bold border-t border-b border-slate-700 py-2">
                ...
                <div class="flex items-center justify-between">
                  ...
                  <span class="font-mono font-extrabold text-2xl text-teal-300">{{ projectedStock }} {{ displayUnit }}</span>
                </div>
              </div>
          ...
          <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-200/80">
            ...
            <div class="space-y-3 max-h-[250px] overflow-y-auto">
              <div v-for="tx in stockTransactions.slice(0, 3)" :key="tx.id" class="p-3.5 bg-slate-50 rounded-2xl border border-slate-200/60 text-sm">
    ...
    <!-- Success Modal -->
    <div v-if="showSuccessModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
      <div class="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl border border-slate-100 text-center space-y-6">
        ...
        <div>
          ...
          <p class="text-base text-slate-600 mt-2">
            ระบบอัปเดตข้อมูลพัสดุในตาราง <code class="bg-slate-100 px-2 py-0.5 rounded text-emerald-700 font-bold">items</code> และบันทึกประวัติลงใน <code class="bg-slate-100 px-2 py-0.5 rounded text-emerald-700 font-bold">stock_transactions</code> เรียบร้อยแล้ว
          </p>
        </div>
