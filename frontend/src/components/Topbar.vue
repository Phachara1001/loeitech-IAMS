...
/* ---------------- แจ้งเตือนบัญชีที่รออนุมัติ (เฉพาะ Admin) ---------------- */
...
// อัปเดตข้อมูลผู้ใช้ทุกครั้งที่มีการเปลี่ยน Route หน้าเว็บ (จะรีเฟรชจำนวนบัญชีรออนุมัติไปด้วยในตัว)
watch(() => route.path, () => {
  loadUserData()
})
...
// ปิดเมนู/แจ้งเตือนเมื่อคลิกนอกพื้นที่
...
<template>
  <header
    ...
    <div class="flex items-center space-x-4">
      ...
      <!-- แจ้งเตือนบัญชีรออนุมัติ (เฉพาะ Admin) -->
      <div v-if="user.role === 'admin'" class="relative" ref="notifRef">
      ...
      <div class="relative" ref="menuRef">
        <button type="button" @click="toggleMenu"
          ...
          <div
            class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold overflow-hidden border border-emerald-200 shrink-0">
          ...
          <ChevronDown class="w-4 h-4 text-slate-400 transition-transform hidden sm:block"
          ...
          <div v-if="isMenuOpen"
            ...
            <div class="px-3.5 py-2.5 border-b border-slate-100">
              ...
              <div class="text-xs text-slate-500 mt-0.5">
                {{ user.role === 'admin' ? 'ผู้ดูแลระบบ' : user.role === 'staff' ? 'เจ้าหน้าที่พัสดุ' :
                'ผู้ใช้งานทั่วไป' }}
              </div>
            </div>
    ...
    <div v-if="isLogoutModalOpen"
      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 relative">
        <button type="button" @click="cancelLogout"
          class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 transition-colors" aria-label="ปิด">
          <X class="w-4 h-4" />
        </button>
