...
const sortBy = ref('name') // name, quantity, unitPrice
...
const filteredItems = computed(() => {
  let result = items.value.filter((item) => {
    ...
    const matchLowStock = !showLowStockOnly.value || (item.quantity <= item.minThreshold && item.quantity > 0)
  ...
  <div class="p-6 lg:p-8 w-full">
    <div
      ...
      <div class="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button v-if="canManage" type="button" @click="openAddForm"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
      <div
        class="bg-white rounded-xl border border-emerald-100 shadow-sm p-4 flex items-center gap-3 hover:shadow-md hover:border-emerald-200 transition-all">
        <div
          class="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-50 to-emerald-100 text-[#065f46] flex items-center justify-center ring-1 ring-emerald-100">
          <Boxes class="w-5 h-5" />
        </div>
        <div>
      ...
      <div
        class="bg-white rounded-xl border border-amber-100 shadow-sm p-4 flex items-center gap-3 hover:shadow-md hover:border-amber-200 transition-all">
        <div
          class="w-11 h-11 rounded-xl bg-gradient-to-br from-amber-50 to-amber-100 text-amber-600 flex items-center justify-center ring-1 ring-amber-100">
      ...
      <div
        class="bg-white rounded-xl border border-red-100 shadow-sm p-4 flex items-center gap-3 hover:shadow-md hover:border-red-200 transition-all">
        <div
          class="w-11 h-11 rounded-xl bg-gradient-to-br from-red-50 to-red-100 text-red-600 flex items-center justify-center ring-1 ring-red-100">
    ...
    <div class="bg-white rounded-2xl border border-emerald-100 shadow-sm overflow-hidden">
      <!-- Toolbar: ค้นหา + กรอง -->
      <div class="flex flex-col xl:flex-row gap-3 p-4 border-b border-emerald-50 bg-emerald-50/20">
        <!-- Search -->
        <div class="relative flex-1">
          <Search class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
          <input v-model="searchQuery" type="text" placeholder="ค้นหาด้วยชื่อหรือรหัสพัสดุ..."
        ...
        <div class="flex flex-col sm:flex-row gap-3">
          ...
          <div class="flex items-center gap-2">
            <div class="relative sm:w-40 shrink-0">
              <select v-model="sortBy"
                class="w-full appearance-none pl-4 pr-8 py-2.5 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#065f46]/20 focus:border-[#065f46] transition">
            ...
            <button @click="toggleSortOrder"
              class="px-3 py-2.5 rounded-lg border border-slate-200 text-slate-600 bg-white hover:bg-slate-50 transition-colors flex items-center justify-center shrink-0 w-12"
          ...
          <!-- Low Stock Toggle -->
          <label
            class="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors shrink-0">
            <input type="checkbox" v-model="showLowStockOnly"
              class="rounded border-slate-300 text-[#065f46] focus:ring-[#065f46]" />
            <span class="text-sm font-medium text-slate-700">เฉพาะพัสดุใกล้หมด</span>
          </label>
        </div>
      </div>
        ...
        <table class="w-full text-sm">
          <thead class="sticky top-0 z-10">
            <tr class="bg-gradient-to-r from-[#065f46] to-[#047857] text-left">
              ...
              <th class="px-4 py-3.5 font-semibold text-[11px] uppercase tracking-wide text-emerald-50 text-right">
                ราคาต่อหน่วย</th>
              <th class="px-4 py-3.5 font-semibold text-[11px] uppercase tracking-wide text-emerald-50 text-right">
                คงเหลือ</th>
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="item in filteredItems" :key="item.id" class="group hover:bg-emerald-50/60 transition-colors">
              <td
                class="px-4 py-3 text-slate-500 font-mono text-xs border-l-4 border-transparent group-hover:border-[#065f46] transition-colors">
                {{ item.sku }}</td>
              <td class="px-4 py-3 text-slate-800 font-medium">{{ item.name }}</td>
              <td class="px-4 py-3 text-slate-500">{{ item.category || '-' }}</td>
              <td class="px-4 py-3 text-right text-slate-500 font-medium">฿{{ item.unitPrice?.toLocaleString() || '-' }}
              </td>
              ...
              <td class="px-4 py-3">
                <span
                  :class="['px-2.5 py-1 rounded-full text-xs font-semibold border shadow-sm', statusOf(item).style]">
                  {{ statusOf(item).label }}
                </span>
              </td>
              <td class="px-4 py-3">
                <div class="flex items-center justify-end gap-1.5">
                  <button type="button" @click="goToHistory(item)" title="ดูประวัติ Running Balance"
                  ...
                  <template v-if="canManage">
                    <button type="button" @click="openEditForm(item)" title="แก้ไข"
                    ...
                    <button type="button" @click="confirmDelete(item)" title="ลบ"
          ...
          <div class="p-6">
            ...
            <div class="space-y-4">
              <div>
                ...
                <input v-model="form.name" type="text" placeholder="เช่น กระดาษ A4 80 แกรม"
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input v-model="form.unit" type="text" placeholder="เช่น รีม, ตลับ"
                    class="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#065f46]/20 focus:border-[#065f46] transition" />
                </div>
              </div>
              ...
              <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input v-model.number="form.minThreshold" type="number" min="0"
                    class="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#065f46]/20 focus:border-[#065f46] transition" />
                </div>
              </div>
