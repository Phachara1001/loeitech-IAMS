  ...
  <div class="min-h-screen w-full flex bg-white font-sarabun">
    ...
    <div class="hidden lg:flex lg:w-[46%] relative overflow-hidden bg-[#072415]">
      ...
      <div class="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div class="absolute -top-20 -right-20 w-[550px] h-[550px] rounded-full bg-[#1B5E3C] opacity-80 blur-[90px]">
        </div>

        <!-- เฉด 2: เขียวตลับเข้ม (ซอยตรงกลาง) -->
        <div class="absolute top-1/3 -left-28 w-[500px] h-[500px] rounded-full bg-[#0F3D26] opacity-90 blur-[80px]">
        </div>

        <!-- เฉด 3: เขียวสดตัดคอนทราสต์ (ตรงกลางฝั่งขวา) ช่วยสร้างจุดนำสายตา -->
        <div class="absolute top-1/2 right-10 w-[350px] h-[350px] rounded-full bg-[#288252] opacity-50 blur-[100px]">
        </div>

        <!-- เฉด 4: เขียวดำลึก (ฐานมุมล่างซ้าย) สร้างความลึกระดับมิติสูงสุด -->
        <div class="absolute -bottom-24 -left-10 w-[500px] h-[500px] rounded-full bg-[#04140B] opacity-95 blur-[70px]">
        </div>
      ...
      <div class="relative z-10 flex flex-col justify-between p-12 xl:p-16 text-white w-full">
        ...
        <div class="flex flex-col items-center text-center">
          <div class="relative mb-4 group cursor-pointer">
            ...
            <div
              class="absolute -inset-3 bg-gradient-to-r from-[#1B5E3C] via-[#288252] to-[#0F3D26] rounded-full blur-md opacity-75 animate-spin-slow group-hover:opacity-100 transition-opacity">
            </div>
            <div class="absolute -inset-1 bg-[#288252] rounded-full blur-sm opacity-60 animate-pulse"></div>

            <img src="/logo1.png" alt="ตราสัญลักษณ์วิทยาลัย"
          ...
          <p class="text-sm font-medium text-emerald-100/80">ระบบสารสนเทศเพื่อการบริหารครุภัณฑ์</p>
          <p class="text-lg font-bold text-white">แผนกเทคโนโลยีสารสนเทศ</p>
        </div>
        ...
        <p class="text-sm text-white/50">© {{ new Date().getFullYear() }} แผนกเทคโนโลยีสารสนเทศ วิทยาลัยเทคนิคเลย</p>
      </div>
    </div>
      ...
      <div class="w-full max-w-md">
        ...
        <div class="flex flex-col items-center text-center mb-10 lg:items-start lg:text-left">
          <div class="relative mb-4 lg:hidden">
            <div
              class="absolute -inset-1.5 bg-gradient-to-r from-[#0F3D26] to-[#1B5E3C] rounded-full blur opacity-60 animate-pulse">
            </div>
            <img src="/logo1.png" alt="ตราสัญลักษณ์วิทยาลัย"
              class="relative w-20 h-20 rounded-full object-cover ring-2 ring-[#1B5E3C]/30 shadow-md" />
          </div>
        ...
        <form class="space-y-6" @submit.prevent="handleSubmit" novalidate>
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input id="password" v-model="password" :type="showPassword ? 'text' : 'password'"
                autocomplete="current-password" placeholder="กรอกรหัสผ่าน"
          ...
          <div v-if="errorMessage"
            class="flex items-start gap-2.5 rounded-xl bg-red-50 border-2 border-red-100 px-4 py-3.5" role="alert">
        ...
        <p class="text-lg text-slate-600 text-center lg:text-left mt-8">
          ...
          <button type="button" @click="handleOpenRegister"
        ...
        <p class="text-base text-slate-500 text-center lg:text-left mt-6">
          พบปัญหาการเข้าใช้งานติดต่อกับผู้ดูแลระบบได้ทันที
        </p>
      </div>
    </div>
    ...
    <!-- ป็อปอัปสร้างบัญชี -->
    <RegisterModal v-if="showRegisterModal" @close="showRegisterModal = false" @registered="handleRegistered" />
  </div>
</template>
